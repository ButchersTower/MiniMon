...
public class BattlePanel extends JPanel implements Runnable, MouseListener,
		KeyListener {
	...
	int[][] stats = { { 20, 20, 20, 10, 20, 10 }, { 20, 20, 16, 15, 20, 12 } };
	...
	void drawBattle() {
		...
		g.drawImage(imageAr[1], enemXX, enemYY, null);
		drawButts();
	}
	...
	void enemyAttack() {
		...
		if (stats[1][0] > 0) {
			...
			if (a == 1) {
				int f = rand.nextInt(100) + 1;
				if (f < attacks[0][1]) {
					int dmg = damageCalc(stats[1][1], stats[0][2],
							attacks[0][0], stats[1][5]);
